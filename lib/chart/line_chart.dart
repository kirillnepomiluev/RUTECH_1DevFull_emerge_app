...
import 'package:emerge/chart/chart_line.dart';
import 'package:emerge/chart/chart_point.dart';
import 'package:emerge/chart/datetime_chart_point.dart';
import 'package:emerge/chart/datetime_series_converter.dart';
import 'package:emerge/chart/highlight_point.dart';
import 'package:emerge/common/dates.dart';
import 'package:emerge/common/pair.dart';
import 'package:emerge/common/text_direction_helper.dart';
...
class LineChart {
  ...
  TextStyle axisTextsStyle;
  double minYValue;
  double maxYValue;


  LineChart(this.lines, this.fromTo, {this.tapTextFontWeight, this.axisTextsStyle = const TextStyle(
      color:  Colors.black87,
      fontWeight: FontWeight.w200,
      fontSize: 10), this.maxYValue, this.minYValue,
  });
  ...
  TextStyle  axisTextsStyle = const TextStyle(
            color:  Colors.black87,
            fontWeight: FontWeight.w200,
            fontSize: 10)
      }) {
    ...
    return LineChart(convertFromDateMaps.left, convertFromDateMaps.right,
        tapTextFontWeight: tapTextFontWeight, axisTextsStyle: axisTextsStyle,
      maxYValue: maxYValue, minYValue: minYValue
    );
  }
  ...
  double minY(String unit) {
    if (minYValue==null) return
    _minY[unit];
    else {
      return min(minYValue, _minY[unit]);
    }
  }
  double maxY(String unit){
    if (maxYValue==null) return
      ...
      return max(maxYValue, _maxY[unit]);
    }
  }
  ...
  void _calcScales(double heightPX) {
    ...
    lines.forEach((line) {
      if (unitToMinMaxY.containsKey(line.unit)) {
        ...
        if (maxYValue!=null)
          maxY = max(maxYValue, maxY);
        if (minYValue!=null)
          minY = min(minYValue, minY);


        unitToMinMaxY[line.unit] = Pair(minY, maxY);
      } else {
        double minY = line.minY;
    ...
    assert(unitToMinMaxY.length <=
        3); //The line chart supports max 3 different units
    ...
    } else if (unitToMinMaxY.length == 3) {
      MapEntry<String, Pair> first = unitToMinMaxY.entries.elementAt(0);
      MapEntry<String, Pair> second = unitToMinMaxY.entries.elementAt(1);
      MapEntry<String, Pair> third = unitToMinMaxY.entries.elementAt(2);

      _minY[first.key] = first.value.left;
      _maxY[first.key] = first.value.right;
      _minY[second.key] = second.value.left;
      _maxY[second.key] = second.value.right;
      _minY[third.key] = second.value.left;
      _maxY[third.key] = second.value.right;

      double firstYScale = ((heightPX - axisOffsetPX) / height(first.key)) *
          _effectiveChartHeightRatio;
      double secondYScale = ((heightPX - axisOffsetPX) / height(second.key)) *
          _effectiveChartHeightRatio;
      double thirdYScale = ((heightPX - axisOffsetPX) / height(third.key)) *
          _effectiveChartHeightRatio;

      _yScales[first.key] = firstYScale;
      _yScales[second.key] = secondYScale;
      _yScales[third.key] = thirdYScale;

      indexToUnit[0] = first.key;
      indexToUnit[1] = second.key;
      indexToUnit[2] = third.key;
    }
  }
